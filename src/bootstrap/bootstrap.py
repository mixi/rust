...
class RustBuild(object):
    ...
    def build_bootstrap(self):
        ...
        if self.get_toml("crt-static", build_section) == "true":
            ...
            target_features += ["-crt-static"]
        if self.get_toml("crt-included", build_section) == "true":
            target_features += ["+crt-included"]
        elif self.get_toml("crt-included", build_section) == "false":
            target_features += ["-crt-included"]
        if target_features:
            env["RUSTFLAGS"] += "-C target-feature=" + (",".join(target_features)) + " "

        linker = self.get_toml("linker", build_section)
